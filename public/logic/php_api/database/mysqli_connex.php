<?php
	...
	function my_connect( $G_DEBUG, $HDUP, $link='' )
	{
		...
		if ( !$link )
		{
			...
			/*
			 * XXX The mysql extension is deprecated and will be
			 * removed in the future: use mysqli or PDO instead.
			 */
			$link = mysqli_connect( $HDUP['host'], $HDUP['user'],
				$HDUP['password'] );
			if ( !$link || !mysqli_select_db($link, $HDUP['database'])) { 
	...
	function my_getInfo( $G_DEBUG, $HDUP, $sql, $link='', $primary_key='' )
	{
		...
		{
			if ( mysqli_error( ) == 'Query was empty' ) { return ''; }

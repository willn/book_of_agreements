<?php
	...
	$Item_Dates = array( );
	$limit_time = '90 day';
	$cmty_num = isset($_GET['cmty']) ? intval($_GET['cmty']) : NULL;
	...
	if (isset($id)) {
		...
		if ( $id == 'committee' ) {
			if (isset($cmty) && intval($cmty)) { $link = "?id=committee&cmty=$cmty"; }
			if (isset($cmty) && intval($sub_cmty_num)) { $link .= "&sub=$sub_cmty_num"; }
		}
	}
